...
def verify(body: schemas.VerifyRequest = Body(...),
    ...
    # An empty baseline (no fields) means no checks can run, which produces a spurious 100/100 PASS
    # for rows that were never examined. Refuse to surface this clearly.
    # Tested on profiles/records rather than count alone: [{}, {}] produces no fields but length > 0.
    if body.baseline_profiles is not None and not body.baseline_profiles:
        raise HTTPException(422, detail={
            "detail": "baseline is empty — no fields to compare against, so no verdict is possible"})
    if body.baseline_records is not None and all(not r for r in body.baseline_records):
        raise HTTPException(422, detail={
            "detail": "baseline is empty — no fields to compare against, so no verdict is possible"})
    ...
    baseline_profiles = (
        {k: v.model_dump() for k, v in body.baseline_profiles.items()}
        if body.baseline_profiles is not None else None
    )
    try:
        outcome = verify_rows(
